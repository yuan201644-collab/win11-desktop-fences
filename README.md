...
- **排序与折叠** —— 框内图标按名称 / 类型 / 修改时间排；点标题栏右侧的箭头按钮（或双击标题栏）把整簇图标收进一条窄标签（图标会暂时藏到屏幕外，桌面立刻清爽），再点一次原样放回。
...
点「整理并显示分组」即可看到：桌面图标按类别聚成一簇簇，每簇外套一个半透明标题框，点标题栏右侧的箭头按钮（或双击标题栏）可把整簇图标收进一条窄标签、桌面瞬间清爽，再点一次图标原样回来；拖动框体则整簇图标跟着走。
